...
public class CheckingServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet CheckingServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            HttpSession session = request.getSession();
            out.println("<h1>Session</h1>");
            out.println("<h1>emailid: " + session.getAttribute("emailid") + "</h1>");
            //out.println("<h1>User Id: " + session.getAttribute("userId") + "</h1>");
            out.println("<h1>User Type: " + session.getAttribute("userType") + "</h1>");
            
            out.println("<h1>Servlet CheckingServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
