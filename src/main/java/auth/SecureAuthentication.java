...
public class SecureAuthentication implements HttpAuthenticationMechanism, Serializable {
    ...
    public AuthenticationStatus validateRequest(HttpServletRequest request, HttpServletResponse response, HttpMessageContext ctx) throws AuthenticationException {
        ...
        try {
            ...
            if (token == null && request.getParameter("emailid") != null) {
                ...
                if (result.getStatus() == Status.VALID) {
                 ...
                 //   KeepRecord.setPassword(password);
                    ...
                    if (result.getCallerGroups().contains("1")) {
                        ...
                        request.getRequestDispatcher("AdminSide/Dashboard.xhtml").forward(request, response);
                        System.out.println("i am in adminrole");
                    }
                    if (result.getCallerGroups().contains("2")) {
                        HttpSession session = request.getSession();
                        session.setAttribute("user", username);
                        request.getRequestDispatcher("AdminSide/uhome.jsf").forward(request, response);
                        System.out.println("i am in userrole");
                    }
                    return status;

                } else {
                    KeepRecord.setErrorStatus("Either Emailid or Password is wrong !");
                    response.sendRedirect("Login.jsf");
